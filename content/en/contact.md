+++
title = 'Report a Case'
description = 'Confidential contact and reporting page.'
translationKey = 'contact'
+++

# Report a Case

Your safety comes first. You may submit a report with minimal personal information. Messages received by the collective are handled with confidentiality and care.

{{< alert >}}
Do not put yourself at risk to collect evidence. If you are monitored or threatened, use a safer channel and seek help from a trusted person.
{{< /alert >}}

## Useful Information

- Your name or pseudonym, if you wish to share it
- A safe contact method, if you agree to be contacted
- What happened
- Where and when it happened
- Victims or witnesses, only if this can be shared safely
- Photos, videos, medical documents, complaint references, or other available material

## Contact

Email: [deboutpourlajustice2@gmail.com](mailto:deboutpourlajustice2@gmail.com)

## Report Form

{{< report-form >}}

If the form does not work on your device, you can also use the direct email option:

{{< button href="mailto:deboutpourlajustice2@gmail.com?subject=Human%20rights%20violation%20report&body=Name%20or%20pseudonym%20%28optional%29%3A%0ASafe%20contact%20%28optional%29%3A%0ALocation%3A%0ADate%3A%0ADescription%20of%20facts%3A%0AFiles%20or%20documents%20available%3A%0ADo%20you%20agree%20to%20be%20contacted%3F%3A%0A" label="Prepare a report email" />}}

## Witness Protection

Before sending files, consider checking whether they contain sensitive metadata, such as GPS location, device name, or the identity of an exposed person.
